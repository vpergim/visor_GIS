...
let datosAuxiliaresListos = false;
let mapaListo = false;
...
async function cargarDatosAuxiliares() {
  ...
  // Los filtros no dependen de que el mapa ni las teselas externas hayan cargado.
  rellenarSelect(
    elementos.ccaa,
    Object.keys(indice).sort(ordenarTexto),
    "Seleccione..."
  ...
  if (mapaListo || mapa.isStyleLoaded()) {
    mapaListo = true;
    mostrarVistaNacional();
    ...
    elementos.status.textContent =
      "Datos cargados. Inicializando el mapa...";
  }
}
...
async function iniciarVisor() {
  try {
    ...
    console.error("Error al cargar los datos auxiliares:", error);
    ...
    elementos.status.innerHTML =
      window.location.protocol === "file:"
        ...
        : "No se pudieron cargar los datos auxiliares del visor. " +
          "Revise la consola del navegador y las rutas de la carpeta <code>data</code>.";
  }
}

// Cargar los JSON inmediatamente. No se debe esperar al evento `load` del mapa,
// porque dicho evento puede retrasarse indefinidamente cuando las teselas base
// externas están bloqueadas o no responden.
iniciarVisor();

function finalizarInicializacionMapa() {
  if (mapaListo) {
    return;
  }
...
if (mapa.isStyleLoaded()) {
  finalizarInicializacionMapa();
} else {
  // `style.load` no espera a que terminen de descargarse las teselas raster.
  mapa.once("style.load", finalizarInicializacionMapa);
}

mapa.on("error", (evento) => {
  // Los fallos de una tesela CARTO no deben impedir cargar los datos locales.
  console.error("Error de MapLibre:", evento?.error ?? evento);
});
